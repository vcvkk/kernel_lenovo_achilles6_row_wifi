...
static void fts_charger_notify_work(struct work_struct *work)
{
    ...
    if (USB_DETECT_IN == fts_usb_detect_flag) {
        if (mutex_lock_interruptible(&input_dev->mutex)) {
            FTS_ERROR("Failed to lock in mutex_lock_interruptible(&input_dev->lock).");
            return;
        }
        ret = fts_write_reg(FTS_REG_CHARGER_MODE_EN, USB_DETECT_IN);
        if (ret < 0) {
            FTS_ERROR("MODE_CHARGER switch to %d fail", USB_DETECT_IN);
        }
        ...
        if (ret < 0) {
            FTS_ERROR("MODE_CHARGER switch to %d fail", USB_DETECT_OUT);
        }
        mutex_unlock(&input_dev->mutex);
    }
}
            ...
            if (ret < 0) {
                ...
                if(fts_usb_detect_flag != charger_detection->usb_connected) {
                    ...
                    if (!ts_data->suspended){
                        queue_work(charger_detection->fts_charger_notify_wq,&charger_detection->charger_notify_work);
                    }
                }
            }
        }
        ...
        if (psy) {
            ...
            if (ret < 0) {
                ...
                if(fts_usb_detect_flag != ts_data->charger_detection->usb_connected) {
                    ...
                    ret = fts_write_reg(FTS_REG_CHARGER_MODE_EN, ts_data->charger_detection->usb_connected);
                    if (ret < 0) {
                    FTS_ERROR("MODE_CHARGER switch to %d fail", ts_data->charger_detection->usb_connected);
                    }
                }
            }
        }
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (of_property_read_bool(client->dev.of_node,"focaltech,usb-charger")) {
        FTS_INFO("focaltech usb_charger set");
