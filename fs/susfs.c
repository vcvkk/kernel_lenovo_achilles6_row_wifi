...
/*
 * try_umount() is an Android-common-kernel helper this susfs4ksu patch
 * assumes already exists; this vendor tree doesn't have it. Implement it
 * using path_umount(), which KernelSU-Next's own Kbuild already patches
 * into fs/namespace.c / fs/internal.h for its own umount hook. The
 * check_wsmount parameter is unused: every caller in this file passes
 * false, and the real Android helper's write-shared-mount check isn't
 * needed for that case.
 */
static int try_umount(const char *mnt, bool check_wsmount, int flags)
{
	struct path path;
	int err;
